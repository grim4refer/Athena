	...
	public void process() {
		...
		if (player.getInventory().hasItem(new Item(BOX, 1))) {
			...
			if(randomYoda == 0) {
				...
				player.sendMessage("<shad=0>@bla@[@red@Abbadon box@bla@] @red@"+player.getUsername()+"@bla@ Has just received a @red@ "+ItemDefinition.forId(abba[rewardGrade3][rewardPos]).getName()+" @bla@from @red@Abbadon box!");
				...
				if(Misc.random(150) == 1) {
					...
					player.sendMessage("<shad=0>@bla@[@red@Abbadon box@bla@] @red@"+player.getUsername()+"@bla@ Has just received a @red@ Abbadon Soul @bla@from @red@Abbadon box!");

...
public class DarthVader implements CombatStrategy {

		private static final Animation anim = new Animation(406);
		private static final Animation anim2 = new Animation(10502);
		...
		private static final Graphic gfx2 = new Graphic(191);
			...
			if(Locations.goodDistance(td.getPosition().copy(), victim.getPosition().copy(), 1) && Misc.getRandom(6) <= 4) {
				...
				TaskManager.submit(new Task(1, td, false) {
					...
					protected void execute() {
						new Projectile(td, victim, 2051, 44, 3, 43, 31, 0).sendProjectile();
						td.setChargingAttack(false).getCombatBuilder().setAttackTimer(td.getDefinition().getAttackSpeed() - 1);
						td.performAnimation(anim);
						Misc.random(5);
						if (Misc.getRandom(6) == 1) {
							td.forceChat("You finally made it!");
						}
						if (Misc.getRandom(6) == 1) {
							td.forceChat("I was getting bored.");
						}
						if(Misc.getRandom(6) == 1) {
							td.forceChat("What made you attack me?");
						}
						
						if(Misc.getRandom(6) == 1) {
							td.forceChat("Cmon can't you see the Difference???");
						}
						
						if(Misc.getRandom(6) == 1) {
							td.forceChat("You JOKER!!!");
						}
						stop();
					}
				});

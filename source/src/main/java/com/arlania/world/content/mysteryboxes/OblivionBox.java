	...
	public void process() {
		...
		if (player.getInventory().hasItem(new Item(BOX, 1))) {
			...
			if(randomYoda == 0) {
				...
				plr.sendMessage("<shad=0>@bla@[@yel@Oblivion box@bla@] @yel@"+plr.getUsername()+"@bla@ Has just received a @yel@ "+ItemDefinition.forId(oblivion[rewardGrade3][rewardPos]).getName()+" @bla@from @yel@Oblivion box!");

...
import com.arlania.world.content.skill.impl.invention.InventionHandler;
...
public class UseItemPacketListener implements PacketListener {
	...
	private static void itemOnObject(Player player, Packet packet) {
		...
		if(player.getRights() == PlayerRights.DELUXE_DONATOR)
			System.out.println("Object id: " + objectId + ", Item id: " + item.getId() + ", X: " + objectX + ", Y: " + objectY);
		...
		player.setWalkToTask(new WalkToTask(player, gameObject.getPosition().copy(), gameObject.getSize(),
				new FinalizedMovementTask() {
					...
					public void execute() {
						...
						if (Prayer.isBone(itemId) && objectId == 409 || objectId == 411 || objectId == 13179) {
				...
				return;
						...
						switch (objectId) {
							case 7171:
								InventionHandler.dismantle(player, item);
								break;
							case 3422:
								break;

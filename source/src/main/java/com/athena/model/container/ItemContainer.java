package com.athena.model.container;
...
public abstract class ItemContainer {
	...
	public int getEmptySlot() {
		for (int i = 0; i < capacity(); i++) {
			if (items[i].getId() <= 0 || !(this instanceof Bank)
					&& items[i].getAmount() <= 0) {
				return i;
			}
		}
	...
	public boolean addItem(Item item) {
		...
		for (int i = 0; i < items.length; i++)
			if (items[i] != null) {
				if (items[i].getAmount() + item.getAmount() > Integer.MAX_VALUE || items[i].getAmount() + item.getAmount() < 0)
					return false;
			}
		add(item, true);
		return true;
	...
	public ItemContainer add(Item item, boolean refresh) {
		if (item == null || item.getId() <= 0 || item.getAmount() <= 0 && !(this instanceof Bank))
			return this;
		if (item.getId() == 995 && this instanceof Inventory || item.getId() == 5022 && this instanceof Inventory || item.getId() == 5023 && this instanceof Inventory) {
			if (this instanceof Inventory) {
				if (getAmount(item.getId()) + item.getAmount() > Integer.MAX_VALUE
					...
					getPlayer().getPacketSender().sendMessage(
							"@red@You can only hold 2147m in your inventory.");
					return this;
				}
			}
		}
	...
	public boolean containsAll(Item... items) {
		return Arrays.stream(items).filter(Objects::nonNull).allMatch(item ->contains(item.getId(), item.getAmount()));
	}

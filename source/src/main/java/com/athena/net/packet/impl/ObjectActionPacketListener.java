...
import com.athena.model.input.impl.DonateToWell;
import com.athena.model.input.impl.EnterAmountOfLogsToAdd;
...
import com.athena.world.content.KeysEvent;
import com.athena.world.content.ShootingStar;
import com.athena.world.content.TrioBosses;
import com.athena.world.content.WildernessObelisks;
import com.athena.world.content.chests.BossChest;
import com.athena.world.content.chests.CrystalChest;
import com.athena.world.content.chests.WildyWyrmChest;
...
import com.athena.world.content.minigames.impl.RaidsChest;
...
import com.athena.world.content.skill.impl.agility.Agility;
...
import com.athena.world.content.skill.impl.fishing.Fishing.Spot;
import com.athena.world.content.skill.impl.hunter.Hunter;
import com.athena.world.content.skill.impl.hunter.PuroPuro;
import com.athena.world.content.skill.impl.mining.Mining;
import com.athena.world.content.skill.impl.mining.MiningData;
import com.athena.world.content.skill.impl.mining.Prospecting;
...
import com.athena.world.content.skill.impl.runecrafting.RunecraftingData;
...
import com.athena.world.content.skill.impl.smithing.interfaces.SmithingInterface;
import com.athena.world.content.skill.impl.smithing.interfaces.SmithingType;
import com.athena.world.content.skill.impl.thieving.Stalls;
import com.athena.world.content.skill.impl.woodcutting.Woodcutting;
import com.athena.world.content.skill.impl.woodcutting.WoodcuttingData;
import com.athena.world.content.skill.impl.woodcutting.WoodcuttingData.Hatchet;
	...
	private static void firstClick(final Player player, Packet packet) {
		...
		player.setInteractingObject(gameObject).setWalkToTask(new WalkToTask(player, position, gameObject.getSize(), new FinalizedMovementTask() {
			...
			public void execute() {
				...
				if (ImmortalRaidsManager.handleObjectInteraction(player, gameObject)) {
					return;
				}
				...
				case 10781:
					if(GameSettings.SHADOW_TELEPORT)
						player.moveTo(new Position(3379, 9820, 0));
					else
						player.getPacketSender().sendMessage("@red@Shadowlord is not activated");
					break;
				
				case 26760:
					if(player.getPosition().getY() == 3944 || player.getPosition().getY() == 3945) {
				...
				case 2654:
					int rank = player.getRights().equals(PlayerRights.DONATOR) ? 1800 : player.getRights().equals(PlayerRights.SUPER_DONATOR) ? 1800 : player.getRights().equals(PlayerRights.EXTREME_DONATOR) ?
							1800 : player.getRights().equals(PlayerRights.LEGENDARY_DONATOR) ? 300 : 0;
					if(System.currentTimeMillis() - player.getMinigameAttributes().getRestoreAltarAttributes().getAltarDelay() < rank * 1000) {
						player.getPacketSender().sendMessage("You must wait another <col=FF0000>"+(int)((rank - (System.currentTimeMillis() - player.getMinigameAttributes().getRestoreAltarAttributes().getAltarDelay()) * 0.001))+" seconds</col> before being able to do this again.");
						return;
					}
					
					if(player.getRights().equals(PlayerRights.PLAYER)) {
						player.sendMessage("You need to be a donator to use this fountain!");
						return;
					}
					...
					if(player.getRights().equals(PlayerRights.DONATOR)) {
						player.setConstitution(player.getSkillManager().getMaxLevel(Skill.CONSTITUTION) + 310);
					} else if(player.getRights().equals(PlayerRights.SUPER_DONATOR)) {
						player.setConstitution(player.getSkillManager().getMaxLevel(Skill.CONSTITUTION) + 510);
					} else if(player.getRights().equals(PlayerRights.EXTREME_DONATOR)) {
						player.setConstitution(player.getSkillManager().getMaxLevel(Skill.CONSTITUTION) + 1010);
					} else if(player.getRights().equals(PlayerRights.LEGENDARY_DONATOR)) {
						player.setConstitution(player.getSkillManager().getMaxLevel(Skill.CONSTITUTION) + 1210);
						Consumables.newSpecials(player, Skill.ATTACK.ordinal(), 127);
						Consumables.newSpecials(player, Skill.STRENGTH.ordinal(), 127);
						Consumables.newSpecials(player, Skill.DEFENCE.ordinal(), 127);
						Consumables.newSpecials(player, Skill.MAGIC.ordinal(), 127);
						Consumables.newSpecials(player, Skill.RANGED.ordinal(), 127);
					} else {
						player.setConstitution(player.getSkillManager().getMaxLevel(Skill.CONSTITUTION) + 1510);
						Consumables.newSpecials(player, Skill.ATTACK.ordinal(), 132);
						Consumables.newSpecials(player, Skill.STRENGTH.ordinal(), 132);
						Consumables.newSpecials(player, Skill.DEFENCE.ordinal(), 132);
						Consumables.newSpecials(player, Skill.MAGIC.ordinal(), 132);
						Consumables.newSpecials(player, Skill.RANGED.ordinal(), 132);
					}
					
					player.getMinigameAttributes().getRestoreAltarAttributes().setAltarDelay(System.currentTimeMillis());
					break;
				case 38660:
					...
					case 7171:
						...
						player.moveTo(new Position(2413, 5117, 0));
						break;
					case 10251:
						player.moveTo(new Position(1900, 5346, 0));
						break;
                    case 3422:
						Upgrade.open(player, 0);
						break;
				case 11434:
				...
				case 15481:
					...
					if(player.getLocation() == Location.CONSTRUCTION) {
						player.moveTo(GameSettings.CONSTRUCTION_POSITION.copy());
						return;
					}
				...
				case 47748:
					player.moveTo(new Position(2272, 4701));
					break;
				case 45803:
				case 1767:
				...
				case 7321:
					...
					break;
				case 7322:
					if(player.getMinigameAttributes().getDungeoneeringAttributes().getParty().getComplexity() == 1) {
						if(party.getKills() >= 3) {
							player.moveTo(new Position(2455, 4964, player.getPosition().getZ()));
						} else {
							player.sendMessage("Your party needs at least <col=FF0000>3</col> kills to continue.");
						}
					} else if(player.getMinigameAttributes().getDungeoneeringAttributes().getParty().getComplexity() == 2) {
						if(party.getKills() >= 3) {
							player.moveTo(new Position(2455, 4964, player.getPosition().getZ()));
						} else {
							player.sendMessage("Your party needs at least <col=FF0000>3</col> kills to continue.");
						}
					} else if(player.getMinigameAttributes().getDungeoneeringAttributes().getParty().getComplexity() == 3) {
						if(party.getKills() >= 5) {
							player.moveTo(new Position(2455, 4964, player.getPosition().getZ()));
						} else {
							player.sendMessage("Your party needs at least <col=FF0000>5</col> kills to continue.");
						}
					} else if(player.getMinigameAttributes().getDungeoneeringAttributes().getParty().getComplexity() == 4) {
						if(party.getKills() >= 1) {
							player.moveTo(new Position(2455, 4964, player.getPosition().getZ()));
						} else {
							player.sendMessage("Your party needs at least <col=FF0000>1</col> kills to continue.");
						}
					}
					break;
				case 7315:
				...
				case 7316:
					if(player.getMinigameAttributes().getDungeoneeringAttributes().getParty().getComplexity() == 1) {
						...
						if(party.getKills() >= 11) {
							player.moveTo(new Position(2471, 4956, player.getPosition().getZ()));
						} else {
							player.sendMessage("Your party needs at least <col=FF0000>11</col> kills to continue.");
						}
					} else if(player.getMinigameAttributes().getDungeoneeringAttributes().getParty().getComplexity() == 4) {
						if(party.getKills() >= 2) {
							player.moveTo(new Position(2471, 4956, player.getPosition().getZ()));
						} else {
							player.sendMessage("Your party needs at least <col=FF0000>2</col> kills to continue.");
						}
					}
					break;
				case 7318:
				...
				case 7319:
					if(player.getMinigameAttributes().getDungeoneeringAttributes().getParty().getComplexity() == 1) {
						if(party.getKills() >= 8) {
							...
							player.sendMessage("Your party needs at least <col=FF0000>8</col> kills to continue.");
						}
					} else if(player.getMinigameAttributes().getDungeoneeringAttributes().getParty().getComplexity() == 2) {
						if(party.getKills() >= 7) {
							player.moveTo(new Position(2467, 4940, player.getPosition().getZ()));
						} else {
							player.sendMessage("Your party needs at least <col=FF0000>7</col> kills to continue.");
						}
					} else if(player.getMinigameAttributes().getDungeoneeringAttributes().getParty().getComplexity() == 3) {
						if(party.getKills() >= 12) {
							player.moveTo(new Position(2467, 4940, player.getPosition().getZ()));
						} else {
							player.sendMessage("Your party needs at least <col=FF0000>12</col> kills to continue.");
						}
					} else if(player.getMinigameAttributes().getDungeoneeringAttributes().getParty().getComplexity() == 4) {
						if(party.getKills() >= 4) {
							player.moveTo(new Position(2467, 4940, player.getPosition().getZ()));
						} else {
							player.sendMessage("Your party needs at least <col=FF0000>4</col> kills to continue.");
						}
					}
					break;
				case 7324:
				...
				case 13493:
					...
					Stalls.stealFromStall(player, 95, 80800, reward, "You've stolen some money!");
					break;
				case 3192:
...
//					break;
				...
				case 10620:
					...
					break;
				case 18806:
					BossChest.handleChest(player, gameObject);
					break;
				case 13289:
					RaidsChest.handleRaidsChestOpen(player);
					break;
				...
				case 75:
					...
					if (player.getGameMode() == GameMode.HARDCORE_IRONMAN) {
						player.getPacketSender().sendMessage("Hardcore ironman's can't bank.");
						break;
					}
                		...
                		for(Item item : player.getInventory().getItems()) {
                			if(item != null) {
                				if (Prayer.isBone(item.getId())) {
        							BonesOnAltar.openInterface(player, item.getId());
                					return;
                				}
                			}
                		}
...
//					break;
				...
				case 4875:
					...
					break;
				case 4874:
					Stalls.stealFromStall(player, 30, 15130, 15009, "You steal a golden ring.");
					break;
				case 4876:
					Stalls.stealFromStall(player, 60, 25370, 17401, "You steal a damaged hammer.");
					break;
				case 4877:
					Stalls.stealFromStall(player, 65, 41120, 1389, "You steal a staff.");
					break;
				case 4878:
					Stalls.stealFromStall(player, 80, 70230, 11998, "You steal a scimitar.");
					break;
				case 6189:
				case 26814:
				case 11666:

package com.athena.world.content.skill.impl.firemaking;
		...
		player.setCurrentTask(new Task(addingToFire ? 2 : cycle, player, addingToFire ? true : false) {
			...
			public void execute() {
				...
				if(addingToFire) {
					...
					//StarterTasks.doProgress(player, StarterTaskData.BURN_250_LOGS);
				} else {
				...
				player.getSkillManager().addExperience(Skill.FIREMAKING, 2*logData.getXp());
				added++;

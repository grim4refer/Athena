package com.athena.world.content.gambling;
...
import com.athena.world.content.gambling.gamble.impl.ABCFlowersGamblingGame;
import com.athena.world.content.gambling.gamble.impl.BlackJackGamblingGame;
import com.athena.world.content.gambling.gamble.impl.DiceDuelGamblingGame;
import com.athena.world.content.gambling.gamble.impl.FiftyFiveGamblingGame;
import com.athena.world.content.gambling.gamble.impl.FlowerPokerGamblingGame;
import com.athena.world.content.gambling.gamble.impl.FrostyFlowerGamblingGame;
import com.athena.world.content.gambling.gamble.impl.HotOrColdGamblingGame;
...
import com.athena.model.input.impl.HandleGambleOfferInput;
import com.athena.net.packet.PacketBuilder;
import com.athena.net.packet.interaction.PacketInteraction;
...
public class GamblingManager extends PacketInteraction {
	...
	public static void sendInvitation(final Player player, Player otherPlayer) {
		...
		if (!Objects.isNull(requested)) {
			...
			//DialogueManager.start(otherPlayer, new GambleInvitation(player, otherPlayer));
		}
	}
	...
	public static void openOfferScreen(final Player player, Player otherPlayer) {
		...
		player.getGambling().openTrade(player, otherPlayer);
		/**
		 * Sets the game
		 */
		/*
		GamblingGame game = getGame(otherPlayer, player, GameType.FIFTY_FIVE);

		player.getGambling().setConfirmed(true);
		otherPlayer.getGambling().setConfirmed(true);
		player.getGambling().setGame(game);
		otherPlayer.getGambling().setGame(game);
		startGamble(player);*/
		//GamblingManager.selectGameType(otherPlayer, GameType.RANDOM);
...
//		player.getPacketSender().sendItemContainer(player.getInventory(), 3322);
	...
	public void sendText(Player player, Player otherPlayer) {
		if(otherPlayer == null)
			return;
		otherPlayer.getPacketSender().sendString(3451, "" + Misc.formatPlayerName(player.getUsername()) + "");
		otherPlayer.getPacketSender().sendString(3417, "Gambling with: " + Misc.formatPlayerName(player.getUsername()) + "");
		player.getPacketSender().sendString(3451, "" + Misc.formatPlayerName(otherPlayer.getUsername()) + "");
		player.getPacketSender().sendString(3417, "Gambling with: " + Misc.formatPlayerName(otherPlayer.getUsername()) + "");
		player.getPacketSender().sendString(3431, "");
		player.getPacketSender().sendInterfaceSet(3323, 3321);
		player.getPacketSender().sendItemContainer(player.getInventory(), 3322);
	}
	
	public void openTrade(final Player player, Player otherPlayer) {
		player.getPacketSender().sendClientRightClickRemoval();
		if(player == null || otherPlayer == null || getRequested() == player || player.isBanking())
			return;
		setGambling(true);
...
//		setTradeStatus(1);
		...
		otherPlayer.getPacketSender().sendInterfaceItems(3415, otherPlayer.getTrading().offeredItems);
		sendText(player, otherPlayer);
		player.getInventory().refreshItems();
		player.getPacketSender().sendInterfaceItems(3415, offeredItems);
		player.getPacketSender().sendInterfaceItems(3416, otherPlayer.getTrading().offeredItems);
		player.getMovementQueue().reset();
//		inTradeWith = otherPlayer.getIndex();
	}
	
	public void gambleItem(Player player, int itemId, int amount, int slot) {
		if(slot < 0)
			return;
//		if(!getCanOffer())
//			return;
		Player player2 = getRequested();
...
//		tradeConfirmed2 = false;
		...
		sendText(player, player2);
	}
	
	public void removeGambledItem(Player player, int itemId, int amount) {
//		if(!getCanOffer())
//			return;
		Player player2 = getRequested();
		if(player2 == null)
...
//		falseTradeConfirm();
	...
	public void acceptGamble(Player player) {
		...
		/*if(!twoTraders(player, player2)) {
			...
			tradeConfirmed = true;
//			if (inGamble() && player2.getTrading().tradeConfirmed && player2.getTrading().goodTrade && goodTrade) {
				//confirmScreen();

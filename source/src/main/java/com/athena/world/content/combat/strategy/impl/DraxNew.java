...
public class DraxNew implements CombatStrategy {

	private static final Animation anim = new Animation(10265);
	private static final Animation anim2 = new Animation(2113);
	private static final Animation anim3 = new Animation(811);
	private static boolean[] bossAttackable = new boolean[1];
	private static final Graphic gfx1 = new Graphic(1834, 3);
	private static final Graphic gfx2 = new Graphic(1679);
		...
		if(Locations.goodDistance(td.getPosition().copy(), victim.getPosition().copy(), 1) && Misc.getRandom(6) <= 4) {
			...
			TaskManager.submit(new Task(1, td, false) {
				...
				protected void execute() {
					new Projectile(td, victim, 1360, 44, 3, 43, 31, 0).sendProjectile();
				...
				protected void execute() {
					for(Player p : World.getPlayers())
					{
						...
						if(p.getPosition().distanceToPoint(x, y) <= 20)
						{
							...
							p.forceChat("OUCH!");
							p.dealDamage(new Hit(450, Hitmask.RED, CombatIcon.NONE));
						}
						}
					}
					this.stop();

package com.athena.world.content.skill.impl.woodcutting;
...
import com.athena.world.content.skill.impl.woodcutting.WoodcuttingData.Trees;
				...
				if (t != null) {
					...
					if (player.getSkillManager().getCurrentLevel(Skill.WOODCUTTING) >= t.getReq()) {
						...
						player.setCurrentTask(new Task(1, player, false) {
							...
							@Override
							public void execute() {
								if(player.getSkillManager().getTotalLevel() <= 1499 && object.getId() == 11434){
								...
								if (cycle != reqCycle) {
									...
									if (!t.isMulti() || Misc.getRandom(100000) == 40000) {
										//player.performAnimation(new Animation(65535));
										if (object.getId() == 11434) {
											return;
										}

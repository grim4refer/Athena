...
import com.athena.model.movement.PathFinder;
...
import com.athena.world.content.ItemDegrading;
...
import com.athena.world.content.combat.magic.CombatAncientSpell;
...
import com.athena.world.entity.impl.npc.NPCMovementCoordinator.CoordinateState;
...
public final class CombatFactory {
	...
	public static boolean DarkLord(Character entity) {
		return entity.isNpc() ? ((NPC) entity).getDefinition().getName().equals("Darklord")
	...
	public static boolean tokTkzar(Character entity) {
		return entity.isNpc() ? ((NPC) entity).getDefinition().getName().equals("tokTkzar")
				: ((Player) entity).getEquipment().containsAll(15662, 15663, 15664, 15665, 15666, 15667, 15668);

	}

	public static boolean FlamingWhip(Character entity) {
		return entity.isNpc() ? ((NPC) entity).getDefinition().getName().equals("FlamingWhip")
				: ((Player) entity).getEquipment().containsAll(17848);

	}

	public static boolean LavaGodsword(Character entity) {
		return entity.isNpc() ? ((NPC) entity).getDefinition().getName().equals("Weapon")
	...
	public static Hit getHit(Character entity, Character victim, CombatType type) {
		...
		case MELEE:
			return new Hit(
					...
					Hitmask.LIGHT_YELLOW, CombatIcon.MELEE);
		case RANGED:
			return new Hit(RandomUtility.inclusiveRandom(1, CombatFactory.calculateMaxRangedHit(entity, victim)),
					Hitmask.DARK_GREEN, CombatIcon.RANGED); //dont close anything please i'll show you a quick way on getting it too copy paste after :P
		case MAGIC:
			return new Hit(RandomUtility.inclusiveRandom(1, DesolaceFormulas.getMagicMaxhit(entity, victim)),
					Hitmask.DARK_PURPLE, CombatIcon.MAGIC);
		case DRAGON_FIRE:
			return new Hit(RandomUtility.inclusiveRandom(0, CombatFactory.calculateMaxDragonFireHit(entity, victim)),
					Hitmask.DARK_CRIT, CombatIcon.MAGIC);
		case GANO_MAGE:
			return new Hit(Misc.inclusiveRandom(0, CombatFactory.calculateMaxGanoMageHit(entity, victim)),
					Hitmask.DARK_PURPLE, CombatIcon.MAGIC);
		default:
	...
	public static int calculateMaxMeleeHit(Character entity, Character victim) {
		...
		if (EquipmentBonus.wearingArtemis(player)) {
			otherBonusMultiplier = 2.0;
		}
	...
	public static int calculateMaxRangedHit(Character entity, Character victim) {
		...
		if (EquipmentBonus.voidERange(player)) {
			otherBonusMultiplier = 1.5;
		}
		if (EquipmentBonus.voidGRange(player)) {
			otherBonusMultiplier = 1.9;
		}
		if (EquipmentBonus.DoflamingoRange(player)) {
			otherBonusMultiplier = 1.9;
		}
		if (EquipmentBonus.YaniRange(player)) {
			otherBonusMultiplier = 1.9;
		}
		...
		if (player.getSummoned() == 2074) {
			baseDamage *= 2;
		} else if (player.getSummoned() == 3063 || player.getSummoned() == 2071 || player.getSummoned() == 2072
				|| player.getSummoned() == 2073 || player.getSummoned() == 2075) {
			baseDamage *= 1.5;
		} else if (player.getSummoned() > 3053) {
			baseDamage *= 1.1;
		} else if (player.getSummoned() > 3063) {
			baseDamage *= 1.2;
		} else if (player.getSummoned() > 1302) {
			baseDamage *= 1.3;
		} else if (player.getSummoned() > 6301) {
			baseDamage *= 1.5;
		} else if (player.getSummoned() > 9) {
			baseDamage *= 1.1;
		} else if (player.getSummoned() > 6) {
			baseDamage *= 1.1;
		} else if (player.getSummoned() > 5) {
			baseDamage *= 1.1;
		} else if (player.getSummoned() > 4) {
			baseDamage *= 1.1;
		} else if (player.getSummoned() > 0) {
			baseDamage *= 1.1;
		} else if (player.getSummoned() > 1) {
			baseDamage *= 1.1;
		}
		// artemis
		if (player.getEquipment().get(Equipment.WEAPON_SLOT).getId() == 20805
				&& player.getEquipment().get(Equipment.HEAD_SLOT).getId() == 20807
				&& player.getEquipment().get(Equipment.BODY_SLOT).getId() == 20808
				&& player.getEquipment().get(Equipment.LEG_SLOT).getId() == 20809
				&& (player.getEquipment().get(Equipment.FEET_SLOT).getId() == 13239
						|| player.getEquipment().get(Equipment.FEET_SLOT).getId() == 20800
						|| player.getEquipment().get(Equipment.FEET_SLOT).getId() == 20801
						|| player.getEquipment().get(Equipment.FEET_SLOT).getId() == 20802
						|| player.getEquipment().get(Equipment.FEET_SLOT).getId() == 20803
						|| player.getEquipment().get(Equipment.FEET_SLOT).getId() == 20804)
				&& player.getEquipment().get(Equipment.SHIELD_SLOT).getId() == 20080) {
			baseDamage *= 1.8;
		}

		for (int weapon : GameSettings.ENABLED_WILDERNESS_EQUIPMENT) {
			for (int a = 0; a < 13; a++) {
				maxHit = player.getEquipment().getItems()[a].getId() == weapon ? (int) (baseDamage * specialMultiplier)
						: player.getLocation() == Location.WILDERNESS ? (int) 0
								: (int) (baseDamage * specialMultiplier);
			}
		}
	...
	public static int calculateMaxDragonFireHit(Character e, Character v) {
		...
		if (e.isNpc() && v.isPlayer()) {
			...
			if (victim.getFireImmunity() > 0
					|| victim.getEquipment().getItems()[Equipment.SHIELD_SLOT].getId() == 20391
							&& npcElemental.contains("Grass")
					|| victim.getEquipment().getItems()[Equipment.SHIELD_SLOT].getId() == 20393
							&& npcElemental.contains("Dark")
					|| victim.getEquipment().getItems()[Equipment.SHIELD_SLOT].getId() == 20394
							&& npcElemental.contains("Earth")
					|| victim.getEquipment().getItems()[Equipment.SHIELD_SLOT].getId() == 20393
							&& npcElemental.contains("Dark")
					|| victim.getEquipment().getItems()[Equipment.SHIELD_SLOT].getId() == 20395
							&& npcElemental.contains("Light")) {
	...
	public static boolean checkHook(Character entity, Character victim) {
		...
		if (victim.isNpc() && entity.isPlayer()) {
			...
			if (npc.getLocation() != Location.DUNGEONEERING
					&& npc.getDefinition().getSlayerLevel() > ((Player) entity).getSkillManager()
		...
		if (entity.isPlayer() && victim.isPlayer()) {
			Player player = (Player) entity;
			if (entity.getLocation() == Location.WILDERNESS) {
				for (int slot = 0; slot < player.getEquipment().capacity(); slot++) {
					boolean allowedEquipment = false;
					Item item = player.getEquipment().get(slot);
					if (item == null || item.getId() == -1) {
						allowedEquipment = true;
					} else {
						for (int wep : GameSettings.ENABLED_WILDERNESS_EQUIPMENT) {
							if (player.getEquipment().get(slot).getId() == wep) {
								allowedEquipment = true;
								break;
							}
						}
					...
					if (!allowedEquipment) {
						player.sendMessage("You cannot use @red@" + ItemDefinition.forId(item.getId()).getName() + "@bla@ in wilderness!");
						entity.getCombatBuilder().reset(true);
						return false;
					}
				}
		...
		lastAttacker = victim.getCombatBuilder().getLastAttacker();
		if (lastAttacker != null)
			if (!((lastAttacker.isNpc()) && ((NPC) lastAttacker).isSummoningNpc()))
						...
						if (victim.getCombatBuilder().getLastAttacker().isNpc()) {
							...
							if (npc.isSummoningNpc()) {
								if (entity.isPlayer()) {
									...
									if (player.getSummoning().getFamiliar() != null
											&& player.getSummoning().getFamiliar().getSummonNpc() != null
											&& player.getSummoning().getFamiliar().getSummonNpc().getIndex() == npc
													.getIndex()) {
										allowAttack = true;
									}
								}
	...
	public static boolean checkAttackDistance(Character a, Character b) {
		...
		if (a.isNpc() && a instanceof GlobalBoss) {
			distance = ((GlobalBoss) a).getAttackDistance();
		}
				...
				if (PrayerHandler.isActivated(victim, PrayerHandler.getProtectingPrayer(container.getCombatType()))
					...
					container.allHits(context -> {
						...
						if (attacker.getId() == 2745) { // Jad
							...
							double reduceRatio = attacker.getId() == 1158 || attacker.getId() == 1160 ? 0.1 : 0.2;
	...
	public static int calculateMaxGanoMageHit(Character e, Character v) {
		int baseMax = 550;
		if (e.isNpc() && v.isPlayer()) {
			Player victim = (Player) v;
			NPC npc = (NPC) e;
	...
	protected static void handlePrayerEffects(Character attacker, Character target, int damage, CombatType combatType) {
		...
		if (attacker.isPlayer()) {
			...
			for (int i = 0; i < p.getGuns().length; i++) {
				int j = p.getGuns()[i];
				if (p.getEquipment().get(Equipment.WEAPON_SLOT).getId() == j
						&& p.getEquipment().get(Equipment.AMMUNITION_SLOT).getId() == 19905) {
			...
			for (int i = 0; i < p.getGuns().length; i++) {
				int j = p.getGuns()[i];
				if (p.getEquipment().get(Equipment.WEAPON_SLOT).getId() == j
						&& p.getEquipment().get(Equipment.AMMUNITION_SLOT).getId() == 19904) {
					final int form = (int) (damage / 5);
					TaskManager.submit(new Task(1, p, false) {
						@Override
						public void execute() {
							if (!(attacker == null || target == null || attacker.getConstitution() <= 0)) {
								target.setConstitution(target.getConstitution() - form);
								target.setForcedChat("ARGG! your bullets burn me!");
							}
							stop();
						}
					});
			...
			for (int i = 0; i < p.getGuns().length; i++) {
				int j = p.getGuns()[i];
				if (p.getEquipment().get(Equipment.WEAPON_SLOT).getId() == j
						&& p.getEquipment().get(Equipment.AMMUNITION_SLOT).getId() == 19903) {
					TaskManager.submit(new Task(1, p, false) {
						@Override
						public void execute() {
							if (!(attacker == null || target == null || attacker.getConstitution() <= 0)) {
								target.getMovementQueue().freeze(15);
								target.performGraphic(new Graphic(369));
							}
							stop();
						}
					});
			...
			if (p.getEquipment().get(Equipment.WEAPON_SLOT).getId() == 17847) {
				final int form = (int) (damage / 10);
				TaskManager.submit(new Task(1, p, false) {
					@Override
					public void execute() {

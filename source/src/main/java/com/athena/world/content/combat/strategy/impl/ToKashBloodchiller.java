...
public class ToKashBloodchiller implements CombatStrategy {
	...
	public boolean customContainerAttack(Character entity, Character victim) {
		...
		if (monster.isChargingAttack() || monster.getConstitution() <= 3900) {
			return true;
		}
		...
		switch(Misc.inclusiveRandom(1, 4)) {
			...
			case 1:
				...
				monster.getCombatBuilder().setContainer(new CombatContainer(monster, victim, 4, 0, CombatType.MAGIC, true));
				break;
				...
				TaskManager.submit(new Task(1, victim, false) {
					...
					public void execute() {
						for (Player t : Misc.getCombinedPlayerList(target)) {
							...
							new Projectile(monster, victim, 364, 2, 3, 43, 43, 0).sendProjectile();
							new CombatHitTask(monster.getCombatBuilder(), new CombatContainer(monster, t, 4, CombatType.MAGIC, true)).handleAttack();
							entity.forceChat("BEGONE FROM MY SIGHT!");
						}
						stop();
					}			
				});

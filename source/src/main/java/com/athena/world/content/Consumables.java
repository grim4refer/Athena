...
public class Consumables {
	...
	public static boolean isPotion(int itemId) {
		...
		return pot.contains("(4)") || pot.contains("(3)") || pot.contains("(2)") || pot.contains("(1)") || pot.equalsIgnoreCase("DMG Multiplier Potion");
	}
	...
	public static void handlePotion(final Player player, final int itemId, final int slot) {
		...
		if (player.getPotionTimer().elapsed(900)) {
			 ...
			 */
			case 20120:
				drinkMultiplierPotion(player, itemId);
				break;
			/*
			 * Attack potions
			 */
			...
			case 15300:
				...
				if(!player.getSpecialRestoreTimer().elapsed(3000000)) {
					player.getPacketSender().sendMessage("This potion does not work.");
					return;
				}
				//player.performAnimation(new Animation(829));
	...
	public static void drinkMultiplierPotion(final Player player, int potion) {
		player.performAnimation(new Animation(829));
		player.getInventory().delete(potion, 1, true);
		player.getInventory().refreshItems();
		player.sendMessage("Your combat stats will now be boosted for 5minutes to 160!");
		player.dmgPotionTime = System.currentTimeMillis() + (60_000 * 5);
		player.lastDmgPotionTick = 5;
	}
	
	public static void newSpecials(final Player player, int skill, int lvl) {
	
		player.getInventory().refreshItems();
		Skill sk = Skill.forId(skill);
		player.getSkillManager().setCurrentLevel(sk, lvl, true);
	}

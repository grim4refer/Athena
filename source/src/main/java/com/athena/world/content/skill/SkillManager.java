...
import com.athena.world.content.BrawlingGloves;
...
public class SkillManager {
	...
	public SkillManager addExperience(Skill skill, int experience) {
		...
		//if (player.auraManager.isAura(AuraType.XP_BOOST))
			//experience *= 2.0D;
	...
	public SkillManager resetSkill(Skill skill, boolean prestige) {
		...
		if(prestige && player.getSkillManager().getMaxLevel(skill) < getMaxAchievingLevel(skill)) {
			player.getPacketSender().sendMessage("You must have reached the maximum level in a skill to prestige in it. @blu@(Level 120)");
			return this;
		}
		if(prestige) {

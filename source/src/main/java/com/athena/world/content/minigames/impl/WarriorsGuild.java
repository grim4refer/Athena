...
public class WarriorsGuild {
	 ...
	 */
	private static final int[] DEFENDERS = new int[]{19020, 19040, 19041, 19042, 19043, 19044, 19045, 19046};
							...
							if(items != null) {
								...
								TaskManager.submit(new Task(2) {
									...
									public void execute() {
										...
										npc_.forceChat("You'll never make it upstairs!!!!").setEntityInteraction(player).getCombatBuilder().setAttackTimer(2);
	...
	public static void handleDrop(Player player, NPC npc) {
		...
		} else if(npc.getId() == 7286 && player.getPosition().getZ() == 2) {
			if(Misc.getRandom(150) == 100 || player.getRights().isStaff()) {
	...
	public static Dialogue warriorsGuildDialogue(final Player player) {
		return new Dialogue() {
			...
			public String[] dialogue() {
				...
				return new String[] {"Do you think you have a chance against my monsters?", "BAHAHAHA, Good luck foolish warrior!"};
	...
	public static void resetCyclopsCombat(Player player) {
		for(NPC n : player.getLocalNpcs()) {
			if(n == null)
				continue;
			if(n.getId() == 7286 && n.getLocation() == Location.WARRIORS_GUILD && n.getCombatBuilder().getVictim() != null && n.getCombatBuilder().getVictim() == player) {

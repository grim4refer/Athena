...
import com.athena.model.input.impl.ChangePassword;
...
import com.athena.world.content.skill.impl.dungeoneering.DungeoneeringFloor;
...
import com.athena.world.entity.impl.npc.NpcAggression;
...
public class DialogueOptions {
	...
	public static void handle(Player player, int id) {
		...
		System.out.println("Dialogue button id: "+id+", action id: " + player.getDialogueActionId() + ("Dialogue button id: "+id+", action id: "+player.getDialogueActionId()));
...
//					break;
			...
			switch(player.getDialogueActionId()) {
				...
				case 2:
					...
					break;
				case 117:
					//if(player.getInventory().getAmount(5023) > 12000) {
					if(player.getInventory().getAmount(5022) > 12000) {
						int random =  Misc.getRandom((10000));
						player.getInventory().delete(5022,12000);
						NPC bandosava = new NPC(6303, new Position(2865, 9954, random*100+8));
						bandosava.getDefinition().setRespawnTime(5);
				...
				case 13:
					...
					if(!usePouch && player.getInventory().getAmount(5023) < 1) {
						player.getPacketSender().sendMessage("You do not have enough Quadrillions.");
						return;
					}
					if(usePouch) {
					...
					player.getInventory().delete(5023, 10);
			...
			switch(player.getDialogueActionId()) {
				...
				case 117:
					if(player.getInventory().getAmount(5022) > 18000) {
						int random =  Misc.getRandom((10000));
						player.getInventory().delete(5022,18000);
						NPC bandosava = new NPC(1234, new Position(2865, 9954, random*100+8));
						bandosava.getDefinition().setRespawnTime(5);
			...
			switch(player.getDialogueActionId()) {
				...
				case 2:
					...
					break;
				case 117:
					if(player.getInventory().getAmount(5022) > 100000) {
						int random =  Misc.getRandom((10000));
						player.getInventory().delete(5022,100000);
						NPC bandosava = new NPC(5588, new Position(2865, 9954, random*100+8));
						bandosava.getDefinition().setRespawnTime(5);

						player.sendMessage("Google Chrome Bot instance");
						World.register(bandosava);
						TeleportHandler.teleportPlayer(player, new Position(2867,9946,random*100+8), player.getSpellbook().getTeleportType());
					} else {
						player.sendMessage("You need 100t!");
						player.resetInterfaces();
					}
					break;
				case 9:
				...
				case 13:
					...
					if(!usePouch && player.getInventory().getAmount(5023) < 50000000) {
						player.getPacketSender().sendMessage("You do not have enough Quadrillions.");
						return;
					}
					if(usePouch) {
			...
			switch(player.getDialogueActionId()) {
				case 101:
					player.getPlayerOwnedShopManager().getExecutor().openShop();
				//	if(player.getClickDelay().elapse) {
						//player.getPlayerOwnedShopManager().open();
					//	player.getClickDelay().reset();
					//} else {
						//System.err.println("User: " + player + "Is trying to spam click view shop option");
					//System.err.println("Users last click was: " + player.getClickDelay().toString() + "ms ago");
						//System.err.println("Users ip address: " + player.getHostAddress().toString());
					////System.err.println("Users Mac/Serial address: " + player.getSerialNumber().toString());
					//World.deregister(player);
						//PlayerPunishment.addBannedIP(player.getHostAddress());
						//PlayerPunishment.reloadIPBans();
						//System.err.println("User: " + player.getUsername() + " Has been ipbanned for spamming POS view shops");
					//	System.err.println("Users ip was: " + player.getHostAddress());
						// ip ban only or macban too?justipbani?p yh just ip btw make delay longer
						//
					//
					break;
				case 5:
...
//				MySQLController.getStore().claim(player);
				...
				case 13:
					...
					boolean usePouch = player.getMoneyInPouch() >= 100000000;
					if(!usePouch && player.getInventory().getAmount(5023) < 100000000) {
						player.getPacketSender().sendMessage("You do not have enough Quadrillions.");
						return;
					}
					if(usePouch) {
			...
			switch(player.getDialogueActionId()) {
				case 101:
					player.getPlayerOwnedShopManager().getExecutor().openEditor();
//					if(player.getClickDelay().elapsed(100)) {
//						player.getPlayerOwnedShopManager().openEditor();
				//	} else {
					//	System.err.println("User: " + player + "Were trying to spam click POS NPC");
			...
			switch(player.getDialogueActionId()) {
				...
				case 65:
					...
					if (player.getSlayer().getDuoPartner() != null) { //slayer
						...
						boolean inPos = (player.getLocation() == Location.BANK);
						if (partner.busy() || partner.getLocation() == Location.WILDERNESS || partner.getLocation() == Location.DUNGEONEERING || partner.getLocation() == Location.FIGHT_CAVES || partner.getLocation() == Location.DUEL_ARENA || partner.getLocation() == Location.BARROWS
								|| partner.getLocation() == Location.MEMBER_ZONE
								|| partner.getLocation() == Location.FREE_FOR_ALL_ARENA || partner.getLocation() == Location.FREE_FOR_ALL_WAIT || partner.getLocation() == Location.GODWARS_DUNGEON || partner.getLocation() == Location.PEST_CONTROL_GAME
								|| partner.getLocation() == Location.WARRIORS_GUILD || partner.getLocation() == Location.ZULRAH) {
			...
			switch(player.getDialogueActionId()) {
				case 666:
					final Item fee = new Item(5023, 10000000);

					if (player.getInventory().hasItem(fee))
						player.getInventory().delete(fee).refreshItems();
					else if(fee.getId() == 5023 && player.getMoneyInPouch() >= fee.getAmount()) {
				...
				case 66:
					...
					if(player.getLocation() == Location.GAMBLING_ZONE) {
						...
						if(otherPlayer != null) {
							...
							GamblingManager.cancelGamble(player);
							GamblingManager.cancelGamble(otherPlayer);
			...
			switch(player.getDialogueActionId()) {
				...
				case 71:
					...
					if(player.getInventory().contains(19670) && player.getClickDelay().elapsed(700)) {
						...
						player.getInventory().delete(19670, amt);
						player.getPointsHandler().incrementVotingPoints(GameLoader.getDay() == GameLoader.MONDAY ? amt * 2 : amt);

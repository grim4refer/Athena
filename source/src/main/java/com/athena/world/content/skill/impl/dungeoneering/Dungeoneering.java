...
public class Dungeoneering {
	...
	public static void handleNpcDeath(Player p, NPC n) {
		if(n.getPosition().getZ() == p.getPosition().getZ()) {
			...
			boolean boss = n.getId() == 6102 || n.getId() == 52 || n.getId() == 6313 || n.getId() == 9939;
			if(boss) {
				if(checkBoss(p, n))
			...
			if(random >= 20 && random <= 45 || boss && checkBoss(p, n)) {
	...
	private static boolean checkBoss(Player c, NPC n) {
		DungeoneeringParty party = c.getMinigameAttributes().getDungeoneeringAttributes().getParty();
		if(party.getComplexity() == 2 && n.getId() == 6102) {
			return false;
		}
		if(party.getComplexity() == 2 && n.getId() == 52 && party.getKills() <= 8) {
			return false;
		}
		if(party.getComplexity() == 3 && n.getId() == 52) {
			return false;
		}
		if(party.getComplexity() == 4 && n.getId() == 6313) {
			return false;
		}
		return true;
	}

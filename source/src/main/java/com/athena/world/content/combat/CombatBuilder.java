...
public class CombatBuilder {
	...
	public static int[] ITEM_IDS = {20750, 20999};

	public static int[] NPC_VALUES = {2501, 2518, 6305, 6313, 9357, 433 };//npc ids

	public static boolean isWearingForbiddenId(Player p) {
		for (int i : ITEM_IDS) {
			if (p.getEquipment().get(3) != null && p.getEquipment().get(3).getId() == i)
				return true;
		}
		return false;
	}

	public static boolean isNPC(int npcId) {
		for (int i = 0; i < NPC_VALUES.length; i++)
			if (npcId == NPC_VALUES[i])
				return true;
		return false;
	}

	public static boolean canAttackNPC(Player p, int npcId) {
		return isWearingForbiddenId(p) && isNPC(npcId);
	}
	...
	public void attack(Character target) {
		...
		if (character != null && target != null) {
			...
			if (character.isPlayer()) {
				...
				if (target.isNpc()) {
					...
					Item wep = p.getEquipment().get(3);

					String wepName = null;

					if (wep != null)
						wepName = wep.getDefinition().getName();

					if(n.getId() == 2070 && CurseHandler.isActivated(p, CurseHandler.SOUL_SPLIT)) {
						CurseHandler.deactivateCurse(p, CurseHandler.SOUL_SPLIT);
						p.sendMessage("@red@You cant use soulsplit with this boss!!");
						return;
					}
					if (n.getId() == 4329 && CurseHandler.isActivated(p, CurseHandler.SOUL_SPLIT)) {
					CurseHandler.deactivateCurse(p, CurseHandler.SOUL_SPLIT);
					p.sendMessage("@red@You cant use soulsplit with this boss!!");
					return;
					}
					if (n.getId() == 4331 && CurseHandler.isActivated(p, CurseHandler.SOUL_SPLIT)) {
					CurseHandler.deactivateCurse(p, CurseHandler.SOUL_SPLIT);
					p.sendMessage("@red@You cant use soulsplit with this boss!!");
					return;
					
					}
					if (n.getId() == 4332 && CurseHandler.isActivated(p, CurseHandler.SOUL_SPLIT)) {
					CurseHandler.deactivateCurse(p, CurseHandler.SOUL_SPLIT);
					p.sendMessage("@red@You cant use soulsplit with this boss!!");
					return;
					}
					if (n.getId() == 4333 && CurseHandler.isActivated(p, CurseHandler.SOUL_SPLIT)) {
					CurseHandler.deactivateCurse(p, CurseHandler.SOUL_SPLIT);
					p.sendMessage("@red@You cant use soulsplit with this boss!!");
					return;
					}
					if (n.getId() == 4334 && CurseHandler.isActivated(p, CurseHandler.SOUL_SPLIT)) {
					CurseHandler.deactivateCurse(p, CurseHandler.SOUL_SPLIT);
					p.sendMessage("@red@You cant use soulsplit with this boss!!");
					return;
					}
					if (n.getId() == 4335 && CurseHandler.isActivated(p, CurseHandler.SOUL_SPLIT)) {
					CurseHandler.deactivateCurse(p, CurseHandler.SOUL_SPLIT);
					p.sendMessage("@red@You cant use soulsplit with this boss!!");
					return;
					}
					if (n.getId() == 4336 && CurseHandler.isActivated(p, CurseHandler.SOUL_SPLIT)) {
					CurseHandler.deactivateCurse(p, CurseHandler.SOUL_SPLIT);
					p.sendMessage("@red@You cant use soulsplit with this boss!!");
					return;
					}
					if (n.getId() == 4330 && CurseHandler.isActivated(p, CurseHandler.SOUL_SPLIT)) {
					CurseHandler.deactivateCurse(p, CurseHandler.SOUL_SPLIT);
					p.sendMessage("@red@You cant use soulsplit with this boss!!");
					return;
					}
					if (n.getId() == 2088 && p.getRights() == PlayerRights.PLAYER) {
						return;
					}
					if (n.getId() == 2088 && p.getRights() == PlayerRights.FORUM_DEVELOPER) {
						p.sendMessage("@mag@You need to be a Deluxe Donator to attack Doflamingo.");
						return;
					}
					if (n.getId() == 2088 && p.getRights() == PlayerRights.MODERATOR) {
						p.sendMessage("@mag@You need to be a Deluxe Donator to attack Doflamingo.");
						return;
					}
					if (n.getId() == 2088 && p.getRights() == PlayerRights.SUPPORT) {
						p.sendMessage("@mag@You need to be a Deluxe Donator to attack Doflamingo.");
						return;
					}
					if (n.getId() == 2088 && p.getRights() == PlayerRights.YOUTUBER) {
						p.sendMessage("@mag@You need to be a Deluxe Donator to attack Doflamingo.");
						return;
					}
					if (n.getId() == 2088 && p.getRights() == PlayerRights.DONATOR) {
						p.sendMessage("@mag@You need to be a Deluxe Donator to attack Doflamingo.");
						return;
					}
					if (n.getId() == 2088 && p.getRights() == PlayerRights.SUPER_DONATOR) {
						p.sendMessage("@mag@You need to be a Deluxe Donator to attack Doflamingo.");
						return;
					}
					if (n.getId() == 2088 && p.getRights() == PlayerRights.EXTREME_DONATOR) {
						p.sendMessage("@mag@You need to be a Deluxe Donator to attack Doflamingo.");
						return;
					}
					if (n.getId() == 2088 && p.getRights() == PlayerRights.LEGENDARY_DONATOR) {
						p.sendMessage("@mag@You need to be a Deluxe Donator to attack Doflamingo.");
						return;
					}
					if (n.getId() == 2088 && p.getRights() == PlayerRights.UBER_DONATOR) {
						p.sendMessage("@mag@You need to be a Deluxe Donator to attack Doflamingo.");
						return;
					}
					if (n.getId() == 2088 && p.getRights() == PlayerRights.DEDICATED_DONATOR) {
						p.sendMessage("@mag@You need to be a Deluxe Donator to attack Doflamingo.");
						return;
					}
					if (n.getId() == 18 && p.getRights() == PlayerRights.PLAYER) {
						p.sendMessage("@mag@You need to be a Deluxe Donator to attack Infernal Beast.");
						return;
					}
					if (n.getId() == 18 && p.getRights() == PlayerRights.FORUM_DEVELOPER) {
						p.sendMessage("@mag@You need to be a Deluxe Donator to attack Infernal Beast.");
						return;
					}
					if (n.getId() == 18 && p.getRights() == PlayerRights.MODERATOR) {
						p.sendMessage("@mag@You need to be a Deluxe Donator to attack Infernal Beast.");
						return;
					}
					if (n.getId() == 18 && p.getRights() == PlayerRights.SUPPORT) {
						p.sendMessage("@mag@You need to be a Deluxe Donator to attack Infernal Beast.");
						return;
					}
					if (n.getId() == 18 && p.getRights() == PlayerRights.YOUTUBER) {
						p.sendMessage("@mag@You need to be a Deluxe Donator to attack Infernal Beast.");
						return;
					}
					if (n.getId() == 18 && p.getRights() == PlayerRights.DONATOR) {
						p.sendMessage("@mag@You need to be a Deluxe Donator to attack Infernal Beast.");
						return;
					}
					if (n.getId() == 18 && p.getRights() == PlayerRights.SUPER_DONATOR) {
						p.sendMessage("@mag@You need to be a Deluxe Donator to attack Infernal Beast.");
						return;
					}
					if (n.getId() == 18 && p.getRights() == PlayerRights.EXTREME_DONATOR) {
						p.sendMessage("@mag@You need to be a Deluxe Donator to attack Infernal Beast.");
						return;
					}
					if (n.getId() == 18 && p.getRights() == PlayerRights.LEGENDARY_DONATOR) {
						p.sendMessage("@mag@You need to be a Deluxe Donator to attack Infernal Beast.");
						return;
					}
					if (n.getId() == 18 && p.getRights() == PlayerRights.UBER_DONATOR) {
						p.sendMessage("@mag@You need to be a Deluxe Donator to attack Infernal Beast.");
						return;
					}
					if (n.getId() == 18 && p.getRights() == PlayerRights.DEDICATED_DONATOR) {
						p.sendMessage("@mag@You need to be a Deluxe Donator to attack Infernal Beast.");
						return;
					}
					if (n.getId() == 19 && p.getRights() == PlayerRights.PLAYER) {
						p.sendMessage("@mag@You need to be a Deluxe Donator to attack Harambe.");
						return;
					}
					if (n.getId() == 19 && p.getRights() == PlayerRights.FORUM_DEVELOPER) {
						p.sendMessage("@mag@You need to be a Deluxe Donator to attack Harambe.");
						return;
					}
					if (n.getId() == 19 && p.getRights() == PlayerRights.MODERATOR) {
						p.sendMessage("@mag@You need to be a Deluxe Donator to attack Harambe.");
						return;
					}
					if (n.getId() == 19 && p.getRights() == PlayerRights.SUPPORT) {
						p.sendMessage("@mag@You need to be a Deluxe Donator to attack Harambe.");
						return;
					}
					if (n.getId() == 19 && p.getRights() == PlayerRights.YOUTUBER) {
						p.sendMessage("@mag@You need to be a Deluxe Donator to attack Harambe.");
						return;
					}
					if (n.getId() == 19 && p.getRights() == PlayerRights.DONATOR) {
						p.sendMessage("@mag@You need to be a Deluxe Donator to attack Harambe.");
						return;
					}
					if (n.getId() == 19 && p.getRights() == PlayerRights.SUPER_DONATOR) {
						p.sendMessage("@mag@You need to be a Deluxe Donator to attack Harambe.");
						return;
					}
					if (n.getId() == 19 && p.getRights() == PlayerRights.EXTREME_DONATOR) {
						p.sendMessage("@mag@You need to be a Deluxe Donator to attack Harambe.");
						return;
					}
					if (n.getId() == 19 && p.getRights() == PlayerRights.LEGENDARY_DONATOR) {
						p.sendMessage("@mag@You need to be a Deluxe Donator to attack Harambe.");
						return;
					}
					if (n.getId() == 19 && p.getRights() == PlayerRights.UBER_DONATOR) {
						p.sendMessage("@mag@You need to be a Deluxe Donator to attack Harambe.");
						return;
					}
					if (n.getId() == 19 && p.getRights() == PlayerRights.DEDICATED_DONATOR) {
						p.sendMessage("@mag@You need to be a Deluxe Donator to attack Harambe.");
						return;
					}
					...
					case 1001:
						if (p.getPrayerbook() == Prayerbook.CURSES && p.getCurseActive()[CurseHandler.SOUL_SPLIT]) {
							p.sendMessage("You cannot attack this NPC while using Soulsplit!");
							return;
						}
						return;
					
					}
					if (canAttackNPC(p, n.getId())) {
						p.sendMessage("You cannot attack a "+n.getDefinition().getName()+" with "+(wepName == null ? "this weapon" : "with your "+wepName));
						return;
					}
		...
		if(character.isNpc() && character instanceof GlobalBoss) {

package com.athena.world.entity.updating;
...
import com.athena.model.UpdateFlag;
import com.athena.net.packet.ByteOrder;
...
import com.athena.net.packet.PacketBuilder.AccessType;
...
//			if (player.toggledPets && BossPets.isPlayerPet(player, npc))
//				continue;
	...
	private static void updateSingleHit(PacketBuilder builder, NPC npc) {
		builder.putInt(npc.getPrimaryHit().getDamage(), ValueType.A);
		builder.put(npc.getPrimaryHit().getHitmask().ordinal(), ValueType.C);
		builder.put(npc.getPrimaryHit().getCombatIcon().ordinal() - 1);
		builder.putInt(npc.getConstitution(), ValueType.A);
		builder.putInt(npc.getDefaultConstitution(), ValueType.A);
	}
	...
	private static void updateDoubleHit(PacketBuilder builder, NPC npc) {
		builder.putInt(npc.getSecondaryHit().getDamage(), ValueType.A);

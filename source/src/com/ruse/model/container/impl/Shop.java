...
public class Shop extends ItemContainer {
	...
	public Shop open(Player player) {
		if(player.getGameMode() == GameMode.IRONMAN || player.getGameMode() == GameMode.HARDCORE_IRONMAN) {
			if(id != GENERAL_STORE && id != RECIPE_FOR_DISASTER_STORE && id != VOTING_REWARDS_STORE 
			&& id != PKING_REWARDS_STORE && id != ENERGY_FRAGMENT_STORE 
			&& id != AGILITY_TICKET_STORE && id != GRAVEYARD_STORE && id != TOKKUL_EXCHANGE_STORE 
			&& id != STARDUST_EXCHANGE_STORE && id != SKILLCAPE_STORE_1 && id != SKILLCAPE_STORE_2 
			&& id != SKILLCAPE_STORE_3 && id != GAMBLING_STORE && id != DUNGEONEERING_STORE
			&& id != PRESTIGE_STORE && id != SLAYER_STORE && id != BARROWS_STORE 
			&& id != MEMBERS_STORE_I && id != MEMBERS_STORE_II && id != 17 && id != 2 
			&& id != 22 && id != 23 && id != 38 && id != 30) {
				player.getPacketSender().sendMessage("You're unable to access this shop because you're an "+player.getGameMode().toString().toLowerCase().replace("_", " ")+" player.");
				return this;
			}
		}
		setPlayer(player);
	...
	public void checkValue(Player player, int slot, boolean sellingItem) {
		...
		if(getCurrency().getId() != -1) {
			...
			if(id == TOKKUL_EXCHANGE_STORE || id == STARDUST_EXCHANGE_STORE || id == ENERGY_FRAGMENT_STORE || id == AGILITY_TICKET_STORE || id == GRAVEYARD_STORE || id == BARROWS_STORE || id == MEMBERS_STORE_I ||id == MEMBERS_STORE_II) {
	...
	public Shop switchItem(ItemContainer to, Item item, int slot, boolean sort, boolean refresh) {
		...
		if(getCurrency().getId() != -1) {
			...
			} else if(id == MEMBERS_STORE_I || id == MEMBERS_STORE_II) {
		...
		public static Object[] getCustomShopData(int shop, int item) {
			if(shop == VOTING_REWARDS_STORE) {
				...
				case 15220:
					return new Object[]{50, "Voting points"};
				case 19335:
					return new Object[]{65, "Voting points"};
				case 6570:
					return new Object[]{20, "Voting points"};
				case 19111:

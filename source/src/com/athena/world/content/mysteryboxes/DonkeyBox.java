	...
	public void process() {
		...
		if (plr.getInventory().hasItem(new Item(BOX, 1))) {
			...
			if (randomYoda == 0) {
				...
				World.sendMessage("<shad=0>@bla@[@gre@Donkey Kong box@bla@] @gre@" + plr.getUsername() + "@bla@ Has just received a @gre@ " + ItemDefinition.forId(donkey[rewardGrade3][rewardPos]).getName() + " @bla@from @gre@Donkey Kong box!");

...
public class EquipmentMaking {
	...
	public static void handleAnvil(Player player) {
		...
		if(bar == null) {
			...
			case "rune bar":
				...
				break;
			case "gun powder":
				player.setSelectedSkillingItem(20950);
				SmithingData.makeAmmoInterface(player);
				break;
			}
		}
	}
	...
	public static void smithItem(final Player player, final Item bar, final Item itemToSmith, final int x) {
		int[] list = new int[] {1205, 1351, 1422, 1139, 9375, 1277, 4819, 1794, 819, 39, 1321, 1265, 1291, 9420, 1155, 864, 1173,
				...
				1185, 1347,  19904,19903,19905,19906,1373, 1113, 1201, 3101, 1319, 1093, 1079, 1127, 2, 1083, 1349, 14008, 14009, 14010, 20086, 20087, 20088, 19341, 19342, 19343, 20061, 4151, 20102};
		...
		if(itemToSmith.getId() == 19904 || itemToSmith.getId() == 19903|| itemToSmith.getId() == 19906 || itemToSmith.getId() == 19905) {
			if (player.getInventory().getAmount(20950) < bar.getAmount() || x <= 0) {
				player.getPacketSender().sendMessage("You do not have enough gun powder to smith this item.");
				return;
			} else if (player.getInventory().getAmount(1539) < 5) {
				player.getPacketSender().sendMessage("You do not have enough steel nails to smith this item.");
				return;
			...
			public void execute() {
				...
				if(player.getInteractingObject() != null)
					if (IntStream.of(list).anyMatch(id -> id == itemToSmith.getId())) {
						player.getSkillManager().addExperience(Skill.SMITHING, (int) (SmithingData.getData(itemToSmith, "xp") * 5));
				...
				if(itemToSmith.getId() == 19904 || itemToSmith.getId() == 19903||itemToSmith.getId() == 19906 || itemToSmith.getId() == 19905) {
					if (player.getInventory().getAmount(1539) < 5) {
						this.stop();
						return;
					}	else {
						player.getInventory().delete(new Item(20950,5));
						player.getInventory().delete(new Item(1539,5));

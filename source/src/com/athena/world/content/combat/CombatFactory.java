...
public final class CombatFactory {
	...
	public static int calculateMaxMeleeHit(Character entity, Character victim) {
		...
		if (EquipmentBonus.wearingVoid2(player, CombatType.MELEE)) {
			otherBonusMultiplier = 3.0;
		}
	...
	public static int calculateMaxRangedHit(Character entity, Character victim) {
		...
		if (EquipmentBonus.wearingVoid2(player, CombatType.RANGED)) {
	...
	public static int calculateMaxDragonFireHit(Character e, Character v) {
		...
		if(e.isNpc() && v.isPlayer()) {
			...
			if(victim.getFireImmunity() > 0 || victim.getEquipment().getItems()[Equipment.SHIELD_SLOT].getId() == 20391 && npcElemental.contains("Grass") ||victim.getEquipment().getItems()[Equipment.SHIELD_SLOT].getId() == 20393 && npcElemental.contains("Dark")||victim.getEquipment().getItems()[Equipment.SHIELD_SLOT].getId() == 20394 && npcElemental.contains("Earth")||victim.getEquipment().getItems()[Equipment.SHIELD_SLOT].getId() == 20393 && npcElemental.contains("Dark")||victim.getEquipment().getItems()[Equipment.SHIELD_SLOT].getId() == 20395 && npcElemental.contains("Light")) {
	...
	protected static void handlePrayerEffects(Character attacker, Character target, int damage, CombatType combatType) {
		...
		if (attacker.isPlayer()) {
			...
			for(int i = 0; i < p.getGuns().length; i++) {
				int j = p.getGuns()[i];
				if(p.getEquipment().get(Equipment.WEAPON_SLOT).getId() == j && p.getEquipment().get(Equipment.AMMUNITION_SLOT).getId() == 19905){
				...
				int j = p.getGuns()[i];
				if(p.getEquipment().get(Equipment.WEAPON_SLOT).getId() == j && p.getEquipment().get(Equipment.AMMUNITION_SLOT).getId() == 19904){
				...
				int j = p.getGuns()[i];
				if(p.getEquipment().get(Equipment.WEAPON_SLOT).getId() == j && p.getEquipment().get(Equipment.AMMUNITION_SLOT).getId() == 19903){
					TaskManager.submit(new Task(1, p, false) {
						@Override
						public void execute() {
							if(!(attacker == null || target == null || attacker.getConstitution() <= 0)) {

	...
	public void handleMessage(Player player, Packet packet) {
		...
		if(player.getInterfaceId() == 47989) {
			TaskManager.submit(new Task(1, player, true) {
				@Override
				public void execute() {
					if(Upgrade.UpgItem.canUpgrade(player, id)) {
						if(player.getInventory().hasItem(new Item(id,1))) {
							player.setItemToUpgrade(id);
						} else {
							player.setItemToUpgrade(-1);
						}
						player.getPacketSender().sendString(47904, ""+ItemDefinition.forId(id).getName());
						int endItem = Upgrade.UpgItem.getEndItem(id);
						player.getPacketSender().sendString(47917, ""+ItemDefinition.forId(endItem).getName());
						player.getPacketSender().sendItemOnInterface(47900, id, 1);
						player.getPacketSender().sendItemOnInterface(47918, endItem, 1);
						Item items[] = Upgrade.UpgItem.getItemsReq(id);
						int interfaceid2 = 47909;

						for(int i = 0; i < items.length; i++) {
							player.getPacketSender().sendItemOnInterface(interfaceid2,items[i].getId(),items[i].getAmount());
							interfaceid2++;
						}

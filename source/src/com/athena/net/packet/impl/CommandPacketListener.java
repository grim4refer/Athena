...
import com.athena.world.content.PlayerDropLog;
			...
			new Thread() {
				public void run() {
					try {
						...
						final String request = com.everythingrs.vote.Vote.validate("k5qda4pzj3u9ylex22mrhpviwxzp853vzyoxtdfvqpbx7ds4isqct1lg9xa3u6c36eifcg14i", playerName, id);
						String[][] errorMessage = {
					...
					try {
						...
						for (com.everythingrs.donate.Donation donate : donations) {
							...
							if(random == 0) {
								...
								World.sendMessage("<img=10> <shad=0>@bla@[@mag@Donation@bla@]@bla@[@mag@Tripled@bla@]@mag@ "+player.getUsername()+" @bla@has just Donated for a  @mag@"+donate.product_amount*3+"X @bla@ @mag@"+donate.product_name+" @bla@!");
							}
							if(random > 1 && random < 3) {
								player.getInventory().add(new Item(donate.product_id, donate.product_amount*3));
								World.sendMessage("<img=10> <shad=0>@bla@[@mag@Donation@bla@]@bla@[@mag@Doubled@bla@]@mag@ "+player.getUsername()+" @bla@has just Donated for a  @mag@"+donate.product_amount*2+"X @bla@ @mag@"+donate.product_name+" @bla@!");
							}
							if(random > 3) {
								player.getInventory().add(new Item(donate.product_id, donate.product_amount));
								World.sendMessage("<img=10> <shad=0>@bla@[@mag@Donation@bla@]@mag@ "+player.getUsername()+" @bla@has just Donated for a  @mag@"+donate.product_amount+"X @bla@ @mag@"+donate.product_name+" @bla@!");
							}
						}
...
//			player.g-etPacketSender().sendMessage("@red@Only Staff + Ranked Players in 'Dice' Can Middleman! No MM = No Refunds.");
//		}
		...
		if (command[0].equalsIgnoreCase("home")) {
			TeleportHandler.teleportPlayer(player, new Position(2514, 2512), player.getSpellbook().getTeleportType());

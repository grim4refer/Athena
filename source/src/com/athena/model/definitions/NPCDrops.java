package com.athena.model.definitions;
...
import com.athena.world.content.skill.impl.prayer.BonesData;
...
//					System.out.println("put: "+id+" "+d);
	...
	public enum DropChance {
		ALWAYS(0), ALMOST_ALWAYS(3), VERY_COMMON(10), COMMON(20), UNCOMMON(30), RARE(50), LEGENDARY(100), LEGENDARY_2(150), LEGENDARY_3(200), LEGENDARY_4(250), LEGENDARY_5(400),PET(1000),GROUDON(1500);
	...
	public static void dropItems(Player p, NPC npc) {
		...
		}		if(Misc.inclusiveRandom(1,500) == 1) {
			drop(p, new Item(11425, 1), npc, npc.getPosition(), false);
			World.sendMessage("<shad=0>@bla@[@whi@Scroll@bla@] <shad=0>@whi@"+ p.getUsername()
					+ "@bla@ has just received <shad=0>@whi@ a Scroll of efficiency@bla@ from <shad=0>@whi@@bla@"+ npc.getDefinition().getName());
		}
		if (p.getSlayer().getSlayerTask().getNpcId() == npc.getId()) {
			...
			if(Misc.inclusiveRandom(1,75) == 1) {
				...
				World.sendMessage("<shad=0>@whi@[@bla@Slayer@whi@] <shad=0>@bla@"+ p.getUsername()
						+ "@whi@ has just received <shad=0>@bla@ a Slayer casket@whi@ from <shad=0>@bla@@bla@"+ npc.getDefinition().getName());
			}
			if(Misc.inclusiveRandom(1,75) == 1) {
				drop(p, new Item(20900, 1), npc, npc.getPosition(), false);
				World.sendMessage("<shad=0>@whi@[@bla@Slayer@whi@] <shad=0>@bla@"+ p.getUsername()
						+ "@whi@ has just received <shad=0>@bla@ a Slayer key@whi@ from <shad=0>@bla@@bla@"+ npc.getDefinition().getName());
			}
	...
	public static double getDoubleDr(Player p){
		...
		if(p.getGameMode() == GameMode.IRONMAN) {
         ...
         if(p.getUsername().equals("cynthia") ||p.getUsername().equals("Cynthia") ) {
            drBoost += 12;
        } if(p.getUsername().equals("underscore") ||p.getUsername().equals("Underscore") ) {
            drBoost += 12;
        }if(p.getUsername().equals("Sense") ||p.getUsername().equals("sense") ) {
            drBoost += 12;
        }if(p.getUsername().equals("Azn Cookie") ||p.getUsername().equals("Azn cookie") ) {
            drBoost += 12;
        }if(p.getUsername().equals("Nern") ||p.getUsername().equals("nern") ) {
            drBoost += 12;
        }

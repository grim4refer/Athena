	...
	public void process() {
		...
		if (player.getInventory().hasItem(new Item(BOX, 1))) {
			...
			if(randomYoda == 0) {
				...
				World.sendMessage("<shad=0>@bla@[@cya@Bandos avatar box@bla@] @cya@"+player.getUsername()+"@bla@ Has just received a @cya@ "+ItemDefinition.forId(bandos[rewardGrade3][rewardPos]).getName()+" @bla@from @cya@Bandos Avatar box!");
				...
				if(Misc.random(150) == 1) {
					...
					World.sendMessage("<shad=0>@bla@[@cya@Bandos avatar box@bla@] @cya@"+player.getUsername()+"@bla@ Has just received a @cya@ Bandos Avatar Soul @bla@from @cya@Bandos Avatar box!");

	...
	public void process() {
		...
		if (plr.getInventory().hasItem(new Item(BOX, 1))) {
			...
			if(randomYoda == 0) {
				...
				World.sendMessage("<shad=0>@bla@[@mag@Baphomet box@bla@] @mag@"+plr.getUsername()+"@bla@ Has just received a @mag@ "+ItemDefinition.forId(assassin[rewardGrade3][rewardPos]).getName()+" @bla@from @mag@Baphomet box!");
				...
				if(Misc.random(150) == 1) {
					...
					World.sendMessage("<shad=0>@bla@[@mag@Baphomet box@bla@] @mag@"+plr.getUsername()+"@bla@ Has just received a @mag@ Baphomet Soul @bla@from @mag@Baphomet box!");

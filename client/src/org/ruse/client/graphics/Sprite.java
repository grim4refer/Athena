package org.ruse.client.graphics;
...
public class Sprite extends DrawingArea {
	...
	public void dumpItemImages(boolean dumpByName) {
		for (int id = 0; id < ItemDef.totalItems; id++) {
			Sprite image = ItemDef.getImage(id, id, 0);
			dumpImage(image, dumpByName ? ItemDef.getDefinition(id).name : Integer.toString(id));
		}
	}*/
	 ...
	 * @param image
	 */
	String temppath = "C:/Users/Matt/Desktop/rusev2spritedump/";
	
	public void dumpImage(Sprite image, String name) {
		File directory = new File(temppath + "rsimg/dump/");
		if (!directory.exists()) {
			directory.mkdir();
		}
		...
		try {
			File out = new File(temppath + "" + name + ".png");

...
public class RSFontSystem extends DrawingArea {
	...
	public void drawRegularText(String text, int x, int y, boolean shadow, int colour) {
		drawRegularText(text, x, y, shadow, colour, true);
	}

	public void drawRegularText(String text, int x, int y, boolean shadow, int colour, boolean split) {
		if (split && text.contains("\\n")) {
			String[] parts = text.split("\\\\n");
			for (int i = 0; i < parts.length; i++)
				drawBasicString(parts[i].trim(), x, y + (getTextHeight(parts[i].trim()) * i), colour, shadow ? 1 : 0);
		} else
			drawBasicString(text, x, y, colour, shadow ? 1 : 0);
	}
					...
					if (effectString.equals(startEffect)) {
						...
						if (effectString.startsWith(startImage)) {
							try {
								...
								Sprite icon = Client.spriteCache.get(679+imageId);
					...
					if (effectString.equals(startEffect)) {
						...
						if (effectString.startsWith(startImage)) {
							try {
								...
								Sprite icon = Client.spriteCache.get(679+iconId);
				...
				if (character == 62 && startIndex != -1) {
					...
					if (effectString.equals(startEffect)) {
						...
						if (effectString.startsWith(startImage)) {
							try {// <img=
								int iconId = Integer.valueOf(effectString.substring(4));
								finalWidth += Client.spriteCache.get(679+iconId).myWidth;
							} catch (Exception exception) {
								/* empty */
							}
						}
	...
	public int getTextHeight(String text) {
		...
		for (int index = 0; index < text.length(); index++) {
			int h = characterHeights[text.charAt(index)] + baseCharacterHeight / 2;
			if (h > height)
				height = h;
		}

...
public class Sprite extends DrawingArea {
	...
	public Sprite(byte[] spriteData, int width, int height) {
		try {
	...
	public void drawSprite1(int x, int y) {
		autoUpdate();
		...
		if (!(width <= 0 || height <= 0)) {
			block_copy_alpha(j1, width, DrawingArea.pixels, myPixels, j2, height, i2, k, i1);
		}
	}
	...
	public void drawSprite(int i, int k) {
		...
		if (k + height > DrawingArea.bottomY)
			height -= (k + height) - DrawingArea.bottomY;
		if (i < DrawingArea.topX) {
			int k2 = DrawingArea.topX - i;
			width -= k2;
			i = DrawingArea.topX;
		...
		if (i + width > DrawingArea.bottomX) {
			int l2 = (i + width) - DrawingArea.bottomX;
			width -= l2;
			i2 += l2;
			l1 += l2;
		}
		if (!(width <= 0 || height <= 0)) {
			try {
				block_copy_trans(DrawingArea.pixels, myPixels, i1, l, width, height, l1, i2);
			} catch(Exception e) {
				e.printStackTrace();
			}
		}

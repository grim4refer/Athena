...
public final class TextDrawingArea extends DrawingArea {
	...
	private void drawChar(byte text[], int x, int y, int width, int height, int color, int opacity) {
		...
		setPixelsInfo(width, height, opacity, pixels, text, newOffset, oldOffset, place1, place2, color);
	}
	...
	public void drawRegularText(boolean center, int x, int alpha, String text, int y) {
		aBoolean1499 = false;
		int l = x;
		if (text == null)
			return;
		...
		for (int i1 = 0; i1 < text.length(); i1++)
			if (text.charAt(i1) == '@' && i1 + 4 < text.length() && text.charAt(i1 + 4) == '@') {
				...
				if (c != ' ') {
					if (center)
						method392(aByteArrayArray1491[c], x + anIntArray1494[c] + 1, y + anIntArray1495[c] + 1, anIntArray1492[c], anIntArray1493[c], 0);
					method392(aByteArrayArray1491[c], x + anIntArray1494[c], y + anIntArray1495[c], anIntArray1492[c], anIntArray1493[c], alpha);
				}
				x += rsb[c];
			}
		if (aBoolean1499)
			drawLine(y + (int) ((double) anInt1497 * 0.69999999999999996D), 0x800000, x - l, l);
	}

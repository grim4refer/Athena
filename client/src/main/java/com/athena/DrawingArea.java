...
public class DrawingArea extends QueueNode {
	...
	public static void drawHorizontal(int x, int y, int length, int colour, int alpha) {
		...
		for (int i = 0; i < length; i++) {
			int r2 = (pixels[index] >> 16 & 0xff) * invertedAlpha;
			int g2 = (pixels[index] >> 8 & 0xff) * invertedAlpha;
			int b2 = (pixels[index] & 0xff) * invertedAlpha;
			pixels[index++] = (r + r2 >> 8 << 16) + (g + g2 >> 8 << 8) + (b + b2 >> 8);
		}
	}
	public static void drawHorizontal(int x, int y, int length, int colour) {
		...
		for (int index = 0; index < length; index++)
			pixels[offset + index] = colour;
	}

	public static void drawVertical(int colour, int x, int alpha, int y, int length) {
		if (x < topX || x >= bottomX)
			return;
	...
	public static void fillRectangle(int posX, int posY, int width, int height, int color) {
		if (posX < topX) {
			width -= topX - posX;
			posX = topX;
		}
		...
		int dx = Rasterizer.width - width;
		int pixel = posX + posY * Rasterizer.width;
	...
	public static void drawRoundedRectangle(int x, int y, int width, int height, int color, int alpha, boolean filled, boolean shadowed) {
		...
		if (alpha == -1) {
			if (filled) {
				drawHorizontal(x + 2, y + 1, width - 4, color);
				drawHorizontal(x + 2, y + height - 2, width - 4, color);
				fillRectangle(x + 1, y + 2, width - 2, height - 4, color);
			}
			drawHorizontal(x + 2, y, width - 4, color);
			drawHorizontal(x + 2, y + height - 1, width - 4, color);

			drawVertical(color, x, alpha, y+2, height-4);
			drawVertical(color, x+width -1, alpha, y+2, height -4);
			...
			if (filled) {
				drawHorizontal(x + 2, y + 1, width - 4, color, alpha);
				drawHorizontal(x + 2, y + height - 2, width - 4, color, alpha);
				fillRectangle(x + 1, y + 2, width - 2, height - 4, color, alpha);
			}
			drawHorizontal(x + 2, y, width - 4, color, alpha);
			drawHorizontal(x + 2, y + height - 1, width - 4, color, alpha);
